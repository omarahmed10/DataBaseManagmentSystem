...
//			}
	...
	public static String[] run(String resourceDir, String testPackage,
			List<String> classes) throws Throwable {
		...
		log.append("UnitTests: " + (tests.isEmpty() ? "none!"
				: Arrays.toString(tests.toArray()))).append("\n");

		log.append("Classes: " + (classes.isEmpty() ? "none!"
				: Arrays.toString(classes.toArray()))).append("\n");
		int failed = 0;
		int total = 0;
		...
		for (Class<?> testClass : tests) {
			...
			try {
				...
				log.append("\tImplementations: " + (impls.isEmpty()
						? "{{{none!}}}" : Arrays.toString(impls.toArray())))
						.append("\n");
			} catch (Throwable e) {
			...
			for (Class<?> impl : impls) {
				log.append("\tTesting Implementation: " + impl.getName())
						.append("\n");
				implementations.add(impl.getName());
				implementation = impl;
				JUnitCore junit = new JUnitCore();
				Result result = junit.run(testClass);
				log.append("\t\t{{{Failed: " + result.getFailureCount())
						.append("\n");
				int i = 0;
				...
				log.append("\t\t[[[Total : " + result.getRunCount())
						.append("]]]\n");
			}
		}
		String impls = "";
		for (String impl : implementations)
			impls += impl + ";";
		return new String[] { (total - failed) + "/" + total, log.toString(),
				impls };
	}
